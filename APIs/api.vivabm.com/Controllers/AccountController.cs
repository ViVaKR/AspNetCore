...
) : ControllerBase
{
    ...
    public async Task<IActionResult> ForgotPassword([FromBody] ForgetPasswordDTO forgetPasswordDTO)
    {
        try
        {
            ...
            // var resetLink = $"https://code.vivabm.com/reset-password?email={user.Email}&token={WebUtility.UrlEncode(token)}";
            var resetLink = $"http://localhost:4456/reset-password?email={user.Email}&token={WebUtility.UrlEncode(token)}";
    ...
    public async Task<IActionResult> ForgetPwd(ForgetPasswordDTO forgetPasswordDTO)
    {
        try
        {
            ...
            var passwordResetLink = $"http://localhost:4456/reset-password?email={user.Email}&token={WebUtility.UrlEncode(token)}";
    ...
    public async Task<IActionResult> ResetPassword([FromBody] ResetPasswordDTO resetPasswordDTO)
    {
        try
        {
            ...
            if (!result.Succeeded)
                return BadRequest(new ResponseModel(ResponseCode.Error, "비밀번호 변경에 실패하였습니다.", result.Errors.Select(x => x.Description).ToArray()));
    ...
    public async Task<IActionResult> ConfirmSendEmail(ConfirmEmailDTO confirmEmailDTO)
    {
        try
        {
            ...
            if (user is null)
                return BadRequest(new ResponseModel(ResponseCode.Error,
                "사용자를 찾을 수 없습니다.",
                confirmEmailDTO.Email));

            var token = await _userManager.GenerateEmailConfirmationTokenAsync(user);

            // var resetLink = $"https://code.vivabm.com/confirm-reply-email?email={user.Email}&token={WebUtility.UrlEncode(token)}";
            var resetLink = $"http://localhost:4456/confirm-reply-email?email={user.Email}&token={WebUtility.UrlEncode(token)}";
