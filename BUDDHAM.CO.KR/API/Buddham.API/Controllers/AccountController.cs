...
namespace Buddham.API.Controllers
{
        ...
        ) : ControllerBase
    {
        ...
        {
            Console.WriteLine($"203 GetPrincipalFromExpiredToken => {_jWTConfig.Key}");
        ...
        public async Task<IActionResult> ForgetPwd(ForgetPasswordDTO forgetPasswordDTO)
        {
            try
            {
                ...
                var passwordResetLink = $"https://<client.domain>/reset-password?email={user.Email}&token={WebUtility.UrlEncode(token)}";

                await _emailService.SendEmailAsync(user.Email!, "비밀번호 초기화", $"클릭하여 비밀번호를 변경하여 주세요. <a href='{passwordResetLink}'>clicking here</a>.");
        ...
        public async Task<IActionResult> ConfrimMyEmail(ConfirmEmailDTO confirmEmailDTO)
        {
            try
            {
                ...
                var conformEmailLink = $"https://<client.domain>/confirm-replay-email?email={user.Email}&token={WebUtility.UrlEncode(token)}";

                await _emailService.SendEmailAsync(user.Email!, "이메일 확인", $"이메일 확인 링크를 클릭하여 주세요. <a href='{conformEmailLink}'>clicking here</a>.");
